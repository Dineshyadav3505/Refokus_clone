...
const Marquees = () => {
  ...
  return (
    <>
    <div  className='py-10 mt-44 w-full relative overflow-hidden  '>
    {images.map(((item, index)  => <Marquee key={index}  imagesurls={item} /> ) )}
    </div>
    <div  className=' -mt-10 mb-56 w-full relative overflow-hidden  '>
    {images.map(((item, index)  => <Marquee2 key={index}  imagesurls={item} /> ) )}
    </div>
